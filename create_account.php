		...
		if (count($errors) == 0) {
			...
			if (isset($_POST['submit'])) {
				...
				if (mysql_num_rows($result) == 1) {
					...
					print "<h3>Account already exists. Try again.";
				} else {

					$sql = "INSERT INTO users (username, password, first_name, email, address, city, state, zip) VALUES ('$username', '$hashed_password', '$first_name', '$email', '$address', '$city', '$state', '$zip') ";
			...
			<div class="row">
				<form action="create_account.php" method="POST" target="">
					...
					<div class="u-pull-left">
						...
						<!--username-->
						<label for="username">
						Username: <input type="text" name="username" id="username" value= "<?php if (isset($_POST['username'])) echo $_POST['username'];?>"></label></br>
						<p style="color: red;"><?php if (isset($errors['username_empty'])) echo $errors['username_empty'];?></p>
						
						<!--password-->
						<label for="password">Password: <input type="password" name="password" id="password" value="<?php if (isset($_POST['password'])) echo $_POST['password']; ?>"></label><br/>
						<p style="color: red;"><?php if (isset($errors['password_empty
						'])) echo $errors['password'];?></p>

						<!--email-->
						<label for="email">Email: <input type="text" name="email" id="email" value="<?php if (isset($_POST['email'])) echo $_POST['email']; ?>"></label><br/>
						<p style="color: red;"><?php if (isset($errors['email_empty'])) echo $errors['email_empty']; ?></p>

						<!-- full_name -->
						<label for="full_name">Full Name: <input type="text" name="full_name" id="full_name" value="<?php if (isset($_POST['full_name'])) echo $_POST['full_name']; ?>"></label><br/>
					...
					<div class="u-pull-right">
						...
						<!-- address -->
						<label for="address">Address: <input type="text" name="address" id="address" value="<?php if(isset($_POST['address'])) echo $_POST['address']; ?>"></label><br/>
	                    <p style="color: red;"><?php if (isset($errors['address_empty'])) echo $errors['address_empty']; ?></p>					
 						
 						<!-- city -->
                        <label for="city">City: <input type="text" name="city" id="city" value="<?php if(isset($_POST['city'])) echo $_POST['city']; ?>"></label><br/>
                        ...
                        <!-- state -->
                        <label for="state">State: <input type="text" name="state" id="state" value="<?php if(isset($_POST['state'])) echo $_POST['state']; ?>"></label><br/>
                        ...
                        <!-- zip -->
                        <label for="zip">Zip: <input type="text" name="zip" id="zip" value="<?php if(isset($_POST['zip'])) echo $_POST['zip']; ?>"></label><br/>
                        ...
                        <input id="button" name="submit" id="submit" type="submit"></input>													
					</div>
				</form>
			 </div>
		</div>
